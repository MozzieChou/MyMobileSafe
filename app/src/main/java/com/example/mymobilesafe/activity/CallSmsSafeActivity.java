...
import com.example.mymobilesafe.db.dao.BlackNumberDao;
import com.example.mymobilesafe.domain.BlackNumber;
        ...
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if(flag==1&&dao.find(number)){
                    Toast.makeText(getApplicationContext(), "要修改的电话号码已经存在",Toast.LENGTH_SHORT).show();
                    return ;
                }
                //如果输入的是空，则直接结束当前方法
                if (TextUtils.isEmpty(number)) {
                    ...
                    if (cb_phone.isChecked() && cb_sms.isChecked()) {
                        ...
                        Toast.makeText(getApplicationContext(), "拦截模式不能为空",Toast.LENGTH_SHORT)
                                .show();
                        return;
                    }
